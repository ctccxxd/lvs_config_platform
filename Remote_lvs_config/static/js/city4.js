 $(function(){ //调用插件
    ...
    $(function(){
        form1 = $('form[name=form1]'),
            prev = $('input[name=cho_lvsnum]' , form1),
            city = $('input[name=cho_targetIP]' , form1),
            area = $('input[name=cho_VIP]' , form1),
            insu = $('input[name=cho_realserver]',form1),
            vale = ['Select LvsMum','Select TargetIP','Select VIP','Select RealServer'];
...
$.fn.citySelect = function(options) {  
 	var defaults = {  
 		setId : ['#aa' , '#bb' , '#cc','#dd'],                //默认id
 		stval : ['Select LvsMum','Select TargetIP','Select VIP','Select RealServer'],     //默认值
 		czemt : 'i',                        					  //存值元素
 	...
 	function slide(str){ //下拉事件
 		...
 		oID.click(function(){
 		    ...
 		    oList.stop(true,true).slideToggle('fast');
 		});
 		closed = function(){
 		    oID.removeClass(oClass);
 		    oList.stop(true,true).slideUp('fast');
 		}
 	...
 	function show(obj){   //大按钮事件
 		$(obj).toggleClass('active').find('ul').slideToggle('fast');
 	}
 	if(opts.intva){
